  ...
  render() {
    ...
    return html`
      ...
      <div class="add-topic-wrapper">
        ...
        <div class="add-topic-block">
          <label for="summary" id="summary-label" class="add-topic-label">${this.i18n["summary"]} *</label>
          <input id="summary"
        ...
        <div id="tag-post-block" class="add-topic-block">
          <div id="tag-post-label" for="tags-select" class="add-topic-label">${this.i18n["tag_topic"]}</div>
          ${this.tags.length > 0 ? html`
          <select id="tags-select" @change=${e => this.selectedTagId = e.target.value}>
            ${this.tags.map(tag => html`
            <option value="${tag.id}">${tag.label}</option>
            `)}
          </select>
          <button type="button" @click=${this.selectTag}>${this.i18n["add_tag"]}</button>
          ` : ""}
          ...
          ${this.topic.tags.map(tag => html`
            <div class="tag">
              <div class="tag-${tag.label}" id="tag-${tag.id}">${tag.label}</div>
          ...
          <div id="topic-visibility-wrapper">
            ...
            ${this.groups ? html`
              <div>
                ...
                @click=${this._setVisibility}
                ?checked=${this.topic.visibility === GROUP}>
                <label for="${GROUP}">
                  ${this.i18n["groups"]}
                </label>
              </div>
              ...
              <div id="add-topic-groups-block">
                ...
                <div class="add-topic-group-block">
                  <input type="checkbox" name="${group.title}" @click=${this.toggleGroup} value="${group.reference}" ?checked=${this.topic.groups.includes(group.reference)}>
        ...
        <div id="post-options-block" class="add-topic-block">
          <fieldset id="topic-options-wrapper">
            <legend id="post-options-label" class="add-topic-label">${this.i18n["post_options"]}</legend>
            ${this.canPin ? html`
            <div>
